...
if [ "$SRC" == "sum" -o "$TGT" == "sum" -a ! -f $MONO_PATH/en ]; then
  ...
  cp ../../../../../../dataset/cleaned/UrIIICompSents/mono.sum ./mono.sum
  cp ../../../../../../dataset/cleaned/allCompSents/mono.sum ./mono1.sum
  # cp ../../../../../../dataset/cleaned/allLineByLine/mono.sum ./mono2.sum
fi
...
if [ "$SRC" == "en" -o "$TGT" == "en" -a ! -f $MONO_PATH/en ]; then
  ...
  cp ../../../../../../dataset/dataToUse/allCompSents/train.eng ./train.en
  cp ../../../../../../dataset/dataToUse/UrIIICompSents/train.eng ./train1.en
  cp ../../../../../../dataset/dataToUse/UrIIILineByLine/train.eng ./train2.en
fi

cd $MONO_PATH

# concatenate monolingual data files
if ! [[ -f $SRC/all.$SRC ]]; then
  python3 ../../../stack.py --src $SRC/ --tgt $SRC/all.sum
fi

if ! [[ -f $TGT/all.$TGT ]]; then
  python3 ../../../stack.py --src $TGT/ --tgt $TGT/all.en
fi
