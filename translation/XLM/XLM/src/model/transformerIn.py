...
class TransformerModel(nn.Module):
    ...
    def fwd(self, x, lengths, causal, all_embs=False, find_emb=True, src_enc=None, src_len=None, positions=None, langs=None, cache=None):
        """
        Inputs:
...
#         assert lengths.max().item() <= slen
        ...
        # langs
        if langs is not None and not all_embs:
            if langs.size() != (slen, bs):
                langs.reshape(slen, bs)
...
#         print('After Embedding: ', tensor.shape)
        ...
        if langs is not None and self.use_lang_emb:
            tensor = tensor + self.lang_embeddings(langs).expand_as(tensor)
#         else:
#             tensor = self.embeddings(x)
#             tensor = self.position_embeddings(tensor).expand_as(tensor)
#             tensor = self.lang_embeddings(tensor)
    ...
    def generate(self, src_enc, src_len, tgt_lang_id, max_len=200, sample_temperature=None):
        ...
        while cur_len < max_len:
            ...
            all_scores.append(F.softmax(scores, dim=1))
        ...
        return generated[:cur_len], gen_len, all_scores
